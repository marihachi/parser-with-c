...
bool parse_decls(scanner_t *s, node_t **result) {
  ...
  *result = NULL;
  ...
  while (scanner_get_kind(s) != T_EOF) {
...
bool parse_decl(scanner_t *s, node_t **result) {
  ...
  *result = NULL;
  ...
  if (!scanner_expect(s, T_IDENTIFIER)) return false;
  token = scanner_get_token(s);
  ty_ptr = token->value_ptr;
  ty_length = token->value_length;
  if (!scanner_next(s)) return false;
  ...
  if (!scanner_next_with(s, T_OPEN_PAREN)) return false;
  if (!scanner_next_with(s, T_CLOSE_PAREN)) return false;
  ...
  if (!scanner_next_with(s, T_OPEN_BRACE)) return false;
  if (!scanner_next_with(s, T_CLOSE_BRACE)) return false;
